...
<div class="search">
    <div>
        <form action="tickets.php" method="get" id="ticketSearchForm">
            <div style="display:flex; gap:10px;" >                
                ...
                <input type="text" name="keywords" size="50" value="<?php echo Format::htmlchars($settings['keywords']); ?>">               
                ...
                <select name="topic_id" class="nowarn" onchange="javascript: this.form.submit(); ">
                    ...
                    foreach (Topic::getHelpTopics(true) as $id => $name) {
                        ...
                        if ($count == 0)
                            continue;
                    ?>
                        <option value="<?php echo $id; ?>" i <?php if ($settings['topic_id'] == $id) echo 'selected="selected"'; ?>>
    ...
    <?php if ($settings['keywords'] || $settings['topic_id'] || $_REQUEST['sort']) { ?>
        <div style="margin-top:10px">
            <strong><a href="?clear" style="color:#ff9f1c">
                    <i class="icon-remove-circle"></i> <?php echo __('Clear all filters and sort'); ?></a>
            </strong>
        </div>
    <?php } ?>
